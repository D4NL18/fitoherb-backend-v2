...
public class AuthController {
    ...
    @Operation(
            ...
            description = "Validates user credentials (email and password) and returns a Bearer Token (JWT) for authorized access."
    )
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Invalid credentials",
                    content = @Content(schema = @Schema(implementation = RestErrorMessage.class),
                            examples = @ExampleObject(
                                    ...
                                    value = "{\"status\": \"UNAUTHORIZED\", \"message\": \"E-mail or password invalid.\"}"
                            ))),
            @ApiResponse(responseCode = "400", description = "Validation failed",
                    content = @Content(schema = @Schema(implementation = RestValidationErrorMessage.class),
                            examples = @ExampleObject(
                                    name = "Field Validation Error",
                                    value = "{\"status\": \"BAD_REQUEST\", \"message\": \"Validation failed for one or more fields\", \"errors\": {\"email\": \"must be a well-formed email address\", \"password\": \"must not be blank\"}}"
                            )))
    })
    @PostMapping("/login")
    ...
    @Operation(
            ...
            description = "Creates a new user account in the system database and returns the location URI of the newly created resource."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "User registered successfully"),
            @ApiResponse(responseCode = "409", description = "Conflict - Duplicate E-mail",
                    content = @Content(schema = @Schema(implementation = RestErrorMessage.class),
                            examples = @ExampleObject(
                                    name = "Email Conflict",
                                    value = "{\"status\": \"CONFLICT\", \"message\": \"E-mail already in use\"}"
                            ))),
            @ApiResponse(responseCode = "400", description = "Validation error",
                    content = @Content(schema = @Schema(implementation = RestValidationErrorMessage.class),
                            examples = @ExampleObject(
                                    name = "Registration Validation Error",
                                    value = "{\"status\": \"BAD_REQUEST\", \"message\": \"Validation failed for one or more fields\", \"errors\": {\"birthDate\": \"must be a past date\", \"role\": \"must not be null\"}}"
                            ))),
            @ApiResponse(responseCode = "500", description = "Internal error",
                    content = @Content(schema = @Schema(implementation = RestErrorMessage.class),
                            examples = @ExampleObject(
                                    name = "Database Error",
                                    value = "{\"status\": \"INTERNAL_SERVER_ERROR\", \"message\": \"An unexpected error occurred. Please contact the administrator.\"}"
                            )))
    })
